    ...
    <section>
        <div class="py-2 my-2 is-flex is-justify-content-center is-flex-direction-column">
            <div class="column is-4 pb-6 has-text-gray has-text-centered has-background-white has-text-right">
                <h1 class="is-size-4">
                    محصولات: {{ $store.getters.Products.ProductCategory.productcategory }}
                </h1>
            </div>
                    ...
                    <div class="px-6">
                        ...
                        <div v-if="item.isavailable == 1">
                            <div class="is-justify-content-center is-flex pb-4">
                                <p class="px-1 has-text-danger" style="font-size: 17px;">
                            ...
                            <div class="columns is-mobile">
                                <div class="column has-text-left">
                                    <div>
                                        <select class="is-size-5 select has-background-white is-rounded-twenty"
                                            placeholder="تعداد" :value="counter(item.pk_product)">
                                ...
                                <p v-if="item.discount != 0" style="width: 50px;border-radius: 30px;"
                                    class="has-text-light has-background-danger has-text-centered">
                                    {{ item.discount }}%
                                </p>
                            </div>
                        </div>
...
export default {
    ...
    async asyncData({ store, params, $axios }) {
        const Products = await $axios.$post("auth/ProductsForMenu", {
            id: params.id,
        });
